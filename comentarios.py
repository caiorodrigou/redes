class IPAddress:
    def __init__(self, ip_str):
        """
        Construtor que recebe IP em formato string:
        - Pode ser formato AAA.BBB.CCC.DDD
        - Ou string de bits de tamanho 32 (ex: '11000000101010000000000100000001')
        """
        if '.' in ip_str:
            # IP no formato decimal pontuado
            parts = ip_str.split('.')
            if len(parts) != 4:
    ...
    def toBits(self):
        """Retorna string dos 32 bits do IP"""
        return self.bits

    def toIPv4(self):
        """Retorna IP no formato AAA.BBB.CCC.DDD"""
        return '.'.join(str(o) for o in self.octets)

    def isMask(self):
        """
        Retorna True se o IP for uma máscara válida.
        Máscara válida: bits 1 contíguos no início, depois só zeros.
        Exemplo: 1111111100000000... ok, 111110111... não ok.
        """
        bits = self.bits
        # primeira vez que aparece 0
        first_zero = bits.find('0')
        if first_zero == -1:
            # todos 1 -> válido
            return True
        # depois de first_zero, não pode ter mais 1
    ...
    def maskBits(self):
        """
        Retorna a quantidade de bits 1 contíguos no início do IP, 
        assumindo que é máscara válida.
        """
        if not self.isMask():
            raise ValueError("Não é uma máscara válida")
        # contar quantos 1's seguidos desde o início
        count = 0
        for b in self.bits:
            if b == '1':
                count += 1
...
class IPToolIF:
    ...
    def isValid(ip: IPAddress) -> bool:
        """
        Valida se o IP está dentro dos padrões (0-255 em cada octeto)
        e formato correto. Como a classe IPAddress já valida na construção,
        só conferimos se octetos estão no intervalo.
        """
        try:
            for o in ip.octets:
                if o < 0 or o > 255:
    ...
    def areSameNet(ip1: IPAddress, ip2: IPAddress, mask: IPAddress) -> bool:
        """
        Retorna True se ip1 e ip2 estiverem na mesma rede com a máscara fornecida
        """
        # IP rede = IP & máscara (bit a bit)
    ...
    def broadcast(ip: IPAddress, mask: IPAddress) -> IPAddress:
        """
        Retorna IP broadcast: IP rede OR com o inverso da máscara
        """
    ...
    def network(ip: IPAddress, mask: IPAddress) -> IPAddress:
        """
        Retorna IP da rede (ip AND máscara)
        """
